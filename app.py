"""
Xplor Data Migration — Validation Tool v2 (Streamlit UI — Local)
=================================================================
Run with:
    py -m streamlit run app.py

For the Azure-deployable version (download buttons, no folder picker) see app_azure.py.
"""
...
with st.sidebar:
    ...
    st.markdown(
        ...
        "This file is required to run validation.",
...
<div class="how-to-step">
  ...
  Choose <strong>Active only</strong> to include only Active children, or <strong>Active + WaitList</strong> to include both. Waitlist rows are fully excluded when Active only is selected.</div>
</div>

<div class="how-to-step">
  <div class="step-num">5</div>
  <div class="step-text"><strong>(Optional) Add existing service files</strong><br>
  Upload or point to CSV/XLSX files from <em>other</em> services already in Xplor. The tool will cross-check parents and flag any duplicates that would create a second profile for the same person.</div>
</div>
...
""")
    ...
    st.markdown("""
When existing service files are provided (Step 4), the tool checks every parent in the migration file against those profiles. A duplicate is flagged when:
- **First name + last name** match, AND
- At least one of **DOB**, **contact number**, or **email** also matches.

Flagged rows are reported as **🔴 ERROR** with the source file and matched fields shown.
Action required: link the parent's children to the existing profile and delete the newly created duplicate.
""")
...
# ─────────────────────────────────────────────────────────────────────────────
# GUARD — service map required before anything else
# ─────────────────────────────────────────────────────────────────────────────

if not uploaded_service_map:
...
# Normalise path so forward/back-slash differences don't produce different run_keys
...
# Cross-check parents against data already in Xplor to detect duplicates.
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# RUN VALIDATION (once per unique file + folder + service map combination)
# ─────────────────────────────────────────────────────────────────────────────

svc_map_bytes = uploaded_service_map.read()
...
# ─────────────────────────────────────────────────────────────────────────────
# GO BUTTON — user must confirm before validation starts
# ─────────────────────────────────────────────────────────────────────────────
...
if st.session_state.get("last_run_key") != run_key:
    with st.spinner("Validating and transforming your file — please wait…"):
        ...
        # Write guard — prevents writing files more than once for the same run_key
        # even if Streamlit re-executes the script mid-interaction
...
with col5:
    st.metric(
        "🔁 Within-Upload Dupes",
        dup_intra_count,
        help="Potential duplicate parents found within the uploaded file itself. See duplicate_parents_report.",
    )
with col6:
    st.metric(
        "🔁 Cross-Service Dupes",
        dup_cross_count,
        help="Potential duplicate parents matched against existing service files. See duplicate_parents_report.",
    )
